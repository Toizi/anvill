...
class ELFParser(ImageParser):
    ...
    def _get_string_table_entry(self, string_table: bytearray, index: int) -> str:
        ...
        remaining_bytes = MAX_STRING_LENGTH if remaining_bytes > MAX_STRING_LENGTH else remaining_bytes
            ...
            for reloc_index in range(0, reloc_entry_count):
                ...
                if is_32_bit:
                    elf_relocation.r_offset = int.from_bytes(
                        section_data[reloc_offset:reloc_offset + 4], byteorder="little", signed=False)

                    elf_relocation.r_info = int.from_bytes(
                        section_data[reloc_offset + 4:reloc_offset + 8], byteorder="little", signed=False)

                    if has_addend:
                        elf_relocation.r_addend = int.from_bytes(
                            section_data[reloc_offset + 8:reloc_offset + 16], byteorder="little", signed=False)

                else:
                    elf_relocation.r_offset = int.from_bytes(
                        section_data[reloc_offset:reloc_offset + 8], byteorder="little", signed=False)
                    ...
                    if has_addend:
                        elf_relocation.r_addend = int.from_bytes(
                            section_data[reloc_offset + 16:reloc_offset + 24], byteorder="little", signed=False)
