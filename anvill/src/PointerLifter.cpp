...
void PointerLifter::ReplaceAllUses(llvm::Value *old_val, llvm::Value *new_val) {
  ...
  to_replace.push_back(std::make_pair(old_inst, new_val));
...
llvm::Value* PointerLifter::visitGetElementPtrInst(llvm::GetElementPtrInst& inst) {
    if (inferred_types.find(&inst) != inferred_types.end()) {
        ...
        llvm::Value* new_pointer = visitInferInst(pointer_inst, inferred_type);
        if (new_pointer->getType() != inferred_type->getType()) {
...
llvm::Value *PointerLifter::visitIntToPtrInst(llvm::IntToPtrInst &inst) {
  ...
  if (auto pointer_inst = llvm::dyn_cast<llvm::Instruction>(pointer_operand)) {
    ...
    if (new_ptr->getType()->isPointerTy()) {
...
llvm::Value *PointerLifter::visitLoadInst(llvm::LoadInst &inst) {
  ...
  if (llvm::Instruction *possible_mem_loc =
    ...
    llvm::Value *maybe_new_addr = visitInferInst(possible_mem_loc, ptr_to_ptr);
  ...
  if (llvm::ConstantExpr *const_expr =
    ...
    llvm::Value *new_const_ptr = visitInferInst(expr_as_inst, ptr_to_ptr);
